#!/usr/bin/env python3
"""
Runs every 5 minutes.
Fetches commodity intraday (5m/1d, last 24h) and weekly (60m/7 calendar days).
Uses 5-minute candles instead of 60-minute — eliminates the 60-min staleness lag.
Weekly uses period1/period2 timestamps instead of range="5d" — commodities trade extended
hours and Yahoo's 5d range only returns ~5 calendar days, leaving weekend data missing.
"""

import json
import os
import time
from datetime import datetime, timezone

import requests

HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
        "AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/124.0.0.0 Safari/537.36"
    )
}
YAHOO_BASE = "https://query1.finance.yahoo.com/v8/finance/chart/"

SYMBOLS = {
    "Gold":        "GC%3DF",
    "Silver":      "SI%3DF",
    "Copper":      "HG%3DF",
    "Aluminum":    "ALI%3DF",
    "Platinum":    "PL%3DF",
    "Palladium":   "PA%3DF",
    "WTI Oil":     "CL%3DF",
    "Brent Crude": "BZ%3DF",
    "Natural Gas": "NG%3DF",
    "RBOB Gasoline": "RB%3DF",
    "Heating Oil":   "HO%3DF",
    "Wheat":         "ZW%3DF",
    "Corn":          "ZC%3DF",
    "Soybeans":      "ZS%3DF",
    "Oats":          "ZO%3DF",
    "Coffee":        "KC%3DF",
    "Sugar":         "SB%3DF",
    "Cocoa":         "CC%3DF",
    "Orange Juice":  "OJ%3DF",
}


def save(path: str, data: object):
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    with open(path, "w") as f:
        json.dump(data, f, separators=(",", ":"))
    size_kb = os.path.getsize(path) // 1024
    print(f"  Saved {path} ({size_kb} KB)")


def fetch_yahoo_chart(symbol: str, interval: str, range_: str | None = None, *, period1: int | None = None) -> list | None:
    try:
        if period1 is not None:
            params = {"interval": interval, "period1": period1, "period2": int(time.time())}
        else:
            params = {"interval": interval, "range": range_}
        r = requests.get(
            f"{YAHOO_BASE}{symbol}",
            params=params,
            headers=HEADERS,
            timeout=15,
        )
        r.raise_for_status()
        result = r.json()["chart"]["result"][0]
        timestamps = result["timestamp"]
        closes = result["indicators"]["quote"][0]["close"]
        return [
            {"x": int(ts) * 1000, "y": round(float(c), 8)}
            for ts, c in zip(timestamps, closes)
            if c is not None
        ]
    except Exception as e:
        print(f"    ERROR {symbol} {interval}: {e}")
        return None


def main():
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    print(f"=== fetch_fast.py (commodities)  {ts} ===")

    cutoff_ms = (int(time.time()) - 86_400) * 1000  # 24h ago

    intraday: dict = {}
    weekly: dict = {}

    for name, symbol in SYMBOLS.items():
        print(f"  {name}")
        pts_i = fetch_yahoo_chart(symbol, "5m", "1d")
        if pts_i:
            intraday[name] = [p for p in pts_i if p["x"] >= cutoff_ms]
        pts_w = fetch_yahoo_chart(symbol, "60m", period1=int(time.time()) - 7 * 86_400)
        if pts_w:
            weekly[name] = pts_w
        time.sleep(0.3)

    now_ms = int(time.time() * 1000)
    save("data/commodities-intraday.json", {"fetched_at": now_ms, "assets": intraday})
    save("data/commodities-weekly.json",   {"fetched_at": now_ms, "assets": weekly})

    print("=== Done ===")


if __name__ == "__main__":
    main()
